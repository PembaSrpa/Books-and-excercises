...
- **Recursive Step:** The logic that reduces the problem toward the base case (e.g., n! = n × (n-1)!).
...
Using the Euclidean algorithm: The greatest common divisor of (p, q) equals the greatest common divisor of (q, p mod q). This is a highly efficient recursive solution.
```python
def gcd(a, b):
    if b == 0:
        return a
...
Practicing incremental development to find the hypotenuse using the formula: c = square root of (x² + y²).
